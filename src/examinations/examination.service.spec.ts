...
import { createExamDto, createQueryRunner, exam1, motorVehicleSeed1, owner, updateExamDto, vehicle } from "../../test/unittest-mock-data";
...
describe("ExaminationService", () => {
  ...
  it("create exam but can not find vehicle", async () => {
    ...
    let res = await service.create(createExamDto)
    expect(res).toBe(-1)
